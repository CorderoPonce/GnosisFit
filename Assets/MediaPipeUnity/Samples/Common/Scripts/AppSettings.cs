...
namespace Mediapipe.Unity.Sample
{
  ...
  public class AppSettings : ScriptableObject
  {
    ...
    {
        WebCamSource source = new WebCamSource(
            _preferredDefaultWebCamWidth,
            _defaultAvailableWebCamResolutions
        );

        // HACK GNOSIS FIT: Buscar y forzar la cámara frontal de Android
        WebCamDevice[] devices = WebCamTexture.devices;
        for (int i = 0; i < devices.Length; i++)
        {
            if (devices[i].isFrontFacing)
            {
                // Le decimos al plugin que use el índice de la cámara selfie
